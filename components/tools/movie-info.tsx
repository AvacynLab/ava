...
const TMDBResult = ({ result }: TMDBResultProps) => {
  ...
  // Vérification ajoutée pour éviter l'erreur si result est undefined
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-6">
          ...
          <div className="flex flex-wrap items-center gap-3 text-black/90 dark:text-white/90">
            <div className="flex items-center gap-2">
              ...
              <span>{media.vote_average != null ? media.vote_average.toFixed(1) : '0.0'}</span>
            </div>
  ...
  return (
    <div className="my-4">
      <motion.div
        ...
        whileTap={{ scale: 1.00 }}
        className="bg-neutral-100 dark:bg-neutral-900 rounded-xl cursor-pointer"
        onClick={() => setShowDetails(true)}
      >
        <div className="flex flex-col sm:flex-row gap-3 p-3 sm:p-4">
          <div className="w-[120px] sm:w-40 mx-auto sm:mx-0 aspect-[2/3] relative rounded-lg">
            {media.poster_path ? (
              <Image
      ...
      {isMobile ? (
        ...
        <Dialog open={showDetails} onOpenChange={setShowDetails} >
          <DialogContent className="max-w-3xl p-0 font-sans">
            <DetailContent />
          </DialogContent>
        </Dialog>
      )}
