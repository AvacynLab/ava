...
mapboxgl.accessToken = 'pk.eyJ1IjoiYXZhY3luIiwiYSI6ImNtNm1wcXgyaTBoeTEycXF3a29yN2d2OGUifQ.9exOhhGkdynyfs9XlwDLiA';
...
const MapComponent = ({
  ...
  useEffect(() => {
    ...
    // Contrôles de zoom / rotation
    mapInstance.current.addControl(
      new mapboxgl.NavigationControl(),
      'top-right'
    );

    // Attribution en bas à gauche, compacte
    mapInstance.current.addControl(
  ...
  useEffect(() => {
    ...
    // Supprime d’abord les marqueurs précédents
          ...
          new mapboxgl.Popup({ offset: 25, closeButton: false })
            .setHTML(`
              <div class="p-2 min-w-[200px] bg-white dark:bg-neutral-800 m-0">
                ...
                ${
                  place.vicinity
                    ? `<p class="text-xs mt-1 text-neutral-600 dark:text-neutral-300">${place.vicinity}</p>`
                    : ''
                }
              </div>
            `)
  ...
  return (
    <div className="map-wrapper overscroll-contain w-full h-[60vh] rounded-t-xl overflow-hidden shadow-lg">
